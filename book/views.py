...
def book_details(request,id):
    ...
    if request.session.get('cart_items'):
      if request.session.get('cart_items').get(str(id)):
        qauntity = request.session.get('cart_items')[str(id)]
    context={
        "book":book,
        "quantity" : quantity
    }
...
def check_out(request):
    ...
    return render(request,"book/check_out.html",content)
...
def get_cart_details(request):
    ...
    if not request.session.get('cart_items'):
        return total_price, cart_details
...
def remove_from_cart(request,id):
    ...
    request.session['cart_item'] = cart_items
